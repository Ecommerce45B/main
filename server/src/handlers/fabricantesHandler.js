const { viewFabricantes, createFabricante, updateFabricante, deleteFabricante } = require('../controllers/fabricanteController');
...
const newFabricanteHandler= async (req, res) => {
    ...
    try {
        ...
        // console.log(error);
        res.status(400).send(`No se pudo crear el registro del fabricante ${nombre} ${descripcion}`)
    }
}
...
const updateFabricanteHandler= async (req, res) => {
    ...
    try {
        const newFabricante = await updateFabricante(id, nombre, descripcion, estado)
        res.status(200).json(newFabricante)
    } catch (error) {
        // console.log(error);
